    ...
    <aside className="flex flex-col h-full bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-700 w-[270px] relative z-20">
      ...
      <nav className="flex-1 px-4 py-2 space-y-1 overflow-y-auto">
        ...
        {navItems.map(({ to, label, icon: Icon }) => (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-colors group ${
                isActive
                  ? 'bg-zinc-900 dark:bg-zinc-100 text-white shadow-sm'
          ...
          >
            {({ isActive }) => (
              <>
                <Icon size={19} className={isActive ? 'text-white' : 'text-zinc-300 group-hover:text-zinc-700 transition-colors'} />
                {label}
              </>
            )}
          </NavLink>
      ...
      <div className="px-4 py-6">
        <div className="bg-zinc-50 dark:bg-zinc-800/30 rounded-2xl p-4 mb-3 border border-zinc-200 dark:border-zinc-700">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-zinc-900 dark:text-zinc-200 dark:text-zinc-900 dark:text-zinc-200 text-sm font-semibold border border-zinc-300 dark:border-zinc-600">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold text-zinc-300 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-all group"
        >
